...
const BetopiaVision: React.FC = () => {
    return (
        <div className='container'>
            <ComponentTitle className=' flex-center pb-6'>Grow with <span className='text-primary'>Betopia’s Vision</span></ComponentTitle>
            <p className='text-center text-[22px] leading-[140%] text-[#2D4E75] font-light'>Principles guide everything we do and help us build a culture where everyone can <br /> thrive and excel professionally.</p>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-[30px] mt-[52px]">
                {
                    BetopiaVisionData.map((data, index) => (
                        <div key={index} className={` ${index === 0 ? " bg-[#FCD7BC]/50" : " bg-[#FEF4ED] "}  h-[322.4px] rounded-[12px]`}>
                            <div className="flex items-center justify-center h-full w-full px-[32px]">
                                <div>
                                    ...
                                    <p className="text-[18px] text-center text-[#797979] leading-[160%] font-normal">{data.description}</p>
                                </div>
                            </div>
                        </div>
                    ))
